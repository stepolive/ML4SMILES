...
class multicollinearity_reduction():
    ...
    def hierarchical_cluster_analysis(self, vertical=False,  *args, **kwargs):
        """
        ...
        if x_label_in_numbers == True:
            if x_list is not None:
                ...
                hierarchy.dendrogram(
                        ...
                        labels = self.x_list, 
                        orientation = 'left', 
                        leaf_rotation = 0, #90 + 180, 
                        leaf_font_size = fontsize1
                        )
                
                if horizontal_line is not None:
                    plt.axvline(x=horizontal_line, color='k', linestyle='--')
                    
                plt.ylabel('Feature', labelpad=-750, fontsize=fontsize2, rotation=90)
                plt.xlabel("Ward's linkage distance", fontsize = fontsize2, rotation=0)
